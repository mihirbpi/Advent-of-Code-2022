...
start = (0,1)
orig_state = (0, start, False, False)
...
while (len(pq) > 0):
    ...
    visited.add(popped)
    mins, s, hit_end_first, hit_start_second = popped
    e_row, e_col = s

    if(not hit_end_first and (e_row, e_col) == end):
        hit_end_first = True

    if (hit_end_first and (e_row, e_col) == start):
        hit_start_second = True

    if (hit_end_first and hit_start_second and (e_row, e_col) == end):
        print(mins)
        break
    ...
    for neighbor in neighbors:
            ...
            heappush(pq, (mins+1, neighbor, hit_end_first, hit_start_second))